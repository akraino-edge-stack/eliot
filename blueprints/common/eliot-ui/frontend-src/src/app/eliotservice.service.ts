...
import { nodeDetails, podDetails, nodesDropDownDetails, 
        serviceDetails, deploymentData, currentDeployInfo,
        currentDeployDetails, dashboardInfo } from './datainterface';
...
export class EliotserviceService {
  ...
  private dashboard_url = './../assets/data/dashboard-test.json';
  ...
  private packageUploadUrl = this.baseUrl+'upload'
  private deployUrl = this.baseUrl+'deploy';
  private historyIdUrl = this.baseUrl+'history/files';
  private nodesArrayUrl = this.baseUrl+'nodesonly';
  ...
  private current_deploy_url = './../assets/data/current-deploy.json';
  ...
  getCurrentDeployInfo(): Observable<currentDeployDetails>{
    return this.http.get<currentDeployDetails>(this.current_deploy_url)
  }
  ...
  postDeployRequest(data,nodeData): Observable<any> {
    debugger;
    return this.http.post<any>(this.deployUrl, data, nodeData);
  }

  getDashboardData(): Observable<dashboardInfo> {
    return this.http.get<dashboardInfo>(this.dashboard_url);
  }
